...
void main(void)
{
	...
	vec3 moon = max(-VdL,0.0) > minMoonCos ? vec3(MoonIntensity) : vec3(0.0);
	float minSunCos = 2.0 / sqrt(4.0 + SunSize * SunSize);
	vec3 sun = SunColor * max(1.0 - exp(-pow(max(VdL,0.0)/minSunCos, SunSmooth * 1e3)), 0.0);
	...
	vec3 color = Rayleigh * phaseR + Mie * phaseM + Ground + (lookToGround ? vec3(0.0) : (VdL > 0.0 ? sun : moon));
	vec3 finalColor = exposition(color, 1.0);
